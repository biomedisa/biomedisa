...
def _build_kernel():
    code = """
    __global__ void Funktion(short *verts1, char *arr1, char *arr2, char *mask2, char *lock1, char *lock2, int xsh1, int ysh1, int zsh1, int xsh2, int ysh2, int zsh2, float *centroid1, float *centroid2, int n_verts1, float alpha, float beta, float gamma) {
        ...
        if (index < n_verts1) {
            ...
            /* correct particle */
            if (arr1[position1]>0 && mask2[position2]==0) {
                arr1[position1] = 0;}
            else if (arr1[position1]>0 && lock2[position2]==1) {
                arr1[position1] = 0;}
            else if (arr1[position1]==0 && lock1[position1]==0) {
                arr1[position1] = arr2[position2];}
...
def _build_inverse_kernel():
    code = """
    __global__ void Funktion(short *verts2, char *arr1, char *arr2, char *mask1, char *lock1, char *lock2, int xsh1, int ysh1, int zsh1, int xsh2, int ysh2, int zsh2, float *centroid1, float *centroid2, int n_verts2, float alpha, float beta, float gamma) {
        ...
        if (index < n_verts2) {
            ...
            /* get vertice */
            float z = (float)(verts2[index*3+0]) - centroid2[0];
            float y = (float)(verts2[index*3+1]) - centroid2[1];
            float x = (float)(verts2[index*3+2]) - centroid2[2];
            ...
            int Z = (int)(verts2[index*3+0]);
            int Y = (int)(verts2[index*3+1]);
            int X = (int)(verts2[index*3+2]);

            int position1 = Nz*xsh1*ysh1 + Nx*xsh1 + Ny;
            int position2 = Z*xsh2*ysh2 + X*xsh2 + Y;

            /* correct particle */
            if (arr2[position2]>0 && mask1[position1]==0) {
                arr2[position2] = 0;}
            else if (arr2[position2]>0 && lock1[position1]==1) {
                arr2[position2] = 0;}
            else if (arr2[position2]==0 && lock2[position2]==0) {
                arr2[position2] = arr1[position1];}
...
def correct_particle(verts1, verts2, centroid1, centroid2, alpha, beta, gamma, arr1, arr2, m1, m2, l1, l2):
    ...
    arr1_gpu = gpuarray.to_gpu(arr1.astype(np.int8))
    arr2_gpu = gpuarray.to_gpu(arr2.astype(np.int8))
    lock1_gpu = gpuarray.to_gpu(l1.astype(np.int8))
    lock2_gpu = gpuarray.to_gpu(l2.astype(np.int8))

    # allocate memory
    m2 = m2.astype(np.int8)
    mask2_gpu = cuda.mem_alloc(m2.nbytes)
    cuda.memcpy_htod(mask2_gpu, m2)
    verts1 = np.array(verts1, dtype=np.int16)
    verts1_gpu = cuda.mem_alloc(verts1.nbytes)
    cuda.memcpy_htod(verts1_gpu, verts1)

    # kernel
    block = (128, 1, 1)
    grid = ((verts1.shape[0]//block[0])+1, 1, 1)
    kernel = _build_kernel()
    kernel(verts1_gpu, arr1_gpu, arr2_gpu, mask2_gpu, lock1_gpu, lock2_gpu,
    ...
    # allocate memory
    mask1_gpu = gpuarray.to_gpu(m1.astype(np.int8))
    verts2_gpu = gpuarray.to_gpu(np.array(verts2, dtype=np.int16))

    # inverse kernel
    grid = ((verts2.shape[0]//block[0])+1, 1, 1)
    kernel = _build_inverse_kernel()
    kernel(verts2_gpu, arr1_gpu, arr2_gpu, mask1_gpu, lock1_gpu, lock2_gpu,
        xsh1, ysh1, zsh1, xsh2, ysh2, zsh2, centroid1_gpu, centroid2_gpu,
        n_verts2, alpha, beta, gamma, block=block, grid=grid)
