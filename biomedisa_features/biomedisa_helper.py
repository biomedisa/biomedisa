...
def np_to_nc(results_dir, labeled_array, reference_dir):
    try:
        import netCDF4
    except:
        raise Exception("netCDF4 not found. please use `pip install netCDF4`")
    offset = 0
    noi = len(glob.glob(reference_dir+'/*.nc'))
    for i in range(noi):
        filepath = reference_dir+f'/block{str(i).zfill(8)}.nc'
        offset = save_nc_block(results_dir+'/'+os.path.basename(filepath), labeled_array, filepath, offset)

def nc_to_np(base_dir):
    try:
        import netCDF4
    except:
        raise Exception("netCDF4 not found. please use `pip install netCDF4`")
    # read volume by volume
    noi = len(glob.glob(base_dir+'/*.nc'))
    for i in range(noi):
        filepath = base_dir+f'/block{str(i).zfill(8)}.nc'
